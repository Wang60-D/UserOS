...
import { LABEL2_ROUTES, type Label2RouteName } from './Label2Routes';

type Nav = StackNavigationProp<RootStackParamList>;

const Label2SwitchRow: React.FC<{ activeRoute: Label2RouteName }> = ({ activeRoute }) => {
  const navigation = useNavigation<Nav>();
  const scrollRef = useRef<ScrollView>(null);
  const [viewportWidth, setViewportWidth] = useState(0);
  const [firstButtonWidth, setFirstButtonWidth] = useState(0);
  const [itemLayouts, setItemLayouts] = useState<
    Partial<Record<Label2RouteName, { x: number; width: number }>>
  >({});
...
export default Label2SwitchRow;
